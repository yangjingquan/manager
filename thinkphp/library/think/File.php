...
class File extends SplFileObject
{
     ...
     */
    public function move($path, $type, $savename = true, $replace = true)
    {
        // 文件上传失败，捕获错误代码
        ...
        // 文件保存命名规则
        $saveName = $this->buildSaveName($savename, $type);
     ...
     * @return string
     */
    protected function buildSaveName($savename, $type)
    {
        if (true === $savename) {
            // 自动生成文件名
            if ($this->rule instanceof \Closure) {
                ...
                switch ($this->rule) {
                    case 'date':
                        $savename = $type . DS .date('Y') . DS .date('m') . DS . md5(microtime(true));
                        break;
                    default:
                        if (in_array($this->rule, hash_algos())) {
                            ...
                            $savename = date('Y') . DS .date('m') . DS .date('d') . DS . md5(microtime(true));
                        }
                }
            }
        } elseif ('' === $savename) {
